...
```typescript
import { providerFunctions } from '@cdktn/provider-time'

new providerFunctions.TimeProviderFunctions(providerLocalName: string)
```
...
```typescript
public durationParse(duration: string): IResolvable
```
...
```typescript
public rfc3339Parse(timestamp: string): IResolvable
```
...
```typescript
public unixTimestampParse(unixTimestamp: number): IResolvable
```
